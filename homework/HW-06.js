...
function replaceFirstLast(str) {
    ...
    let mid = str.slice(1, str.length - 2);
    if(str.length >= 2) {
...
function checkAge(num) {
    if(num > 2008 && num < 2024) {
        return "AGE IS NOT ALLOWED";
    }
    else if (num < 2008 && num > 1904) {
...
function secondMax(arr) {
    ...
    return secondMax === Math.max(...arr) ? max : secondMax;
}
...
function secondMin(arr) {
    ...
    return secondMin === Math.min(...arr) ? min : secondMin;
}
